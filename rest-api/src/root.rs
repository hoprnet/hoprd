...
fn collect_hopr_metrics() -> Result<String, ApiErrorStatus> {
    hopr_lib::api::types::telemetry::gather_all_metrics()
        .map(|metrics| {
            metrics
                .lines()
                .filter(|line| {
...
mod tests {
    ...
    async fn collect_metrics_filters_out_session_metrics() -> Result<()> {
        ...
        let session_metric = hopr_lib::api::types::telemetry::MultiCounter::new(
            &session_metric_name,
            "session endpoint filtering test",
            &["session_id"],
        )?;

        let non_session_metric = hopr_lib::api::types::telemetry::MultiCounter::new(
